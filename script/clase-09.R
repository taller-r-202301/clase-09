...
#### **1.1 Obtener la ruta de los archivos:**
...
## crear objeto con la ruta a los archivos dentro de las carpetas
rutas <- list.files(path = "input" , recursive = T , full.names = T)
rutas

#### **1.2 Leer los archivos:**

## rutas con los archivos de las caracteristicas generales
str_subset(string = rutas , pattern = "Caracter")

## rutas: caracteristicas generales y urbanas
rutas %>% 
str_subset(string = . , pattern = "Caracter") %>% 
str_subset(pattern = "Cabecera")

rutas %>% str_subset(pattern = "Caracter") %>% str_subset(pattern = "Cabecera")

rutas %>% str_subset("Caracter") %>% str_subset("Cabecera")

## leer archivos 
cg_urb <- import_list(file = rutas %>% str_subset("Caracter") %>% str_subset("Cabecera"))
class(cg_urb)

## apilar archivos: caracteristicas generales y urbanas
cg_urb <- rbindlist(l = cg_urb , use.names = T , fill = T)
cg_urb$urbano <- 1
class(cg_urb)
dim(cg_urb) 

## ocupados y urbanos
ocu_urb <- import_list(file = rutas %>% str_subset("Ocup") %>% str_subset("Cabe")) %>% 
           rbindlist(l=., use.names=T , fill=T) %>% mutate(ocupado=1)

## desocupados y urbanos
des_urb <- import_list(file = rutas %>% str_subset("Deso") %>% str_subset("Cabe")) %>% 
           rbindlist(l=., use.names=T , fill=T) %>% mutate(desocupado=1)

## fuerza y urbanos
f_urb <- import_list(file = rutas %>% str_subset("Fuerz") %>% str_subset("Cabe")) %>% 
         rbindlist(l=., use.names=T , fill=T) %>% mutate(fuerza=1)

## caracteristicas generales y rural
cg_rur <- import_list(file = rutas %>% str_subset("Caract") %>% str_subset("Resto")) %>% 
          rbindlist(l=., use.names=T , fill=T) %>% mutate(urbano=0)

## ocupados yy rural
ocu_rur <- import_list(file = rutas %>% str_subset("Ocup") %>% str_subset("Resto")) %>% 
           rbindlist(l=., use.names=T , fill=T) %>% mutate(ocupado=1)

## desocupados y rural
des_rur <- import_list(file = rutas %>% str_subset("Deso") %>% str_subset("Resto")) %>% 
           rbindlist(l=., use.names=T , fill=T) %>% mutate(desocupado=1)

## fuerza y rural
f_rur <- import_list(file = rutas %>% str_subset("Fuerz") %>% str_subset("Resto")) %>% 
         rbindlist(l=., use.names=T , fill=T) %>% mutate(fuerza=1)

#### **1.3 Unir en los objetos por tipo:**

## caracteristicas
cg <- bind_rows(cg_urb,cg_rur) %>% clean_names()
table(cg$urbano)

## ocupados
ocu <- bind_rows(ocu_urb,ocu_rur) %>% clean_names()

## desocupados
des <- bind_rows(des_urb,des_rur) %>% clean_names()

## fuerza
fuerza <- bind_rows(f_urb,f_rur) %>% clean_names()

#### **1.4 Unir los objetos en un conjunto de datos:**

## join: c("directorio","secuencia_p","orden"))
geih <- left_join(x = cg , y = ocu , by = c("directorio","secuencia_p","orden")) %>%
        left_join(y = des , by = c("directorio","secuencia_p","orden")) %>% 
        left_join(y = fuerza , by = c("directorio","secuencia_p","orden"))
dim(geih)

## exportar
export(geih , "output/geih_2019_2021.rds")

##=====================================##
## **[2.] Visualizaciones** `ggplot()` ##
##=====================================##
